my role Buf[::T = Int] is Iterable does Stringy does Positional {
    has T @.contents;
    ...
    multi method unpack($template) {
        ...
        for $template.comb(/<[a..zA..Z]>[<[0..9]>+|'*']?/) -> $unit {
...
multi infix:<~|>(Buf $a, Buf $b) {
    ...
    my @ored-contents;
    for ^$minlen {
        @ored-contents.push: $a.contents[$_] +| $b.contents[$_]
    }
...
multi infix:<~^>(Buf $a, Buf $b) {
    ...
    my @xored-contents;
    for ^$minlen {
        @xored-contents.push: $a.contents[$_] +^ $b.contents[$_]
    }
    ...
    for $template.comb(/<[a..zA..Z]>[\d+|'*']?/) -> $unit {
        ...
        given $directive {
            ...
            when 'H' {
                ...
                @bytes.push: map { $_.Numeric.base(16) }, $hexstring.comb(/../);
            }
            when 'C' {
            ...
            when 'L' | 'V' {
                ...
                @bytes.push: $number % 0x100;
                @bytes.push: ($number +> 0x08) % 0x100;
                @bytes.push: ($number +> 0x10) % 0x100;
                @bytes.push: ($number +> 0x18) % 0x100;
            }
            when 'n' {
                my $number = shift(@items);
